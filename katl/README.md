...
2026.9.0-beta.2 and Kubernetes v1.36.4. `cluster.yaml` is the retained node
...
The directory-backed network file sets require a newer `katlctl` than beta.2.
They have been validated with the directory-file-set development build; update
`version` to a release containing that support before using the pinned task
wrapper for this configuration. The OS-side file format is unchanged.
...
task katl:identity
# Back up the Kubernetes identity and Katl's separately reported management
# authority off-cluster. Neither belongs in Git or public PXE storage.
```

`katl/.local`, `*.katlkey`, `*.katlcfg`, and `artifacts/` are ignored. Generated
bundles contain private node management keys. Retain the reported management
CA key as well as `katl/.local/hollywoo-kubernetes.katlkey`; they serve different
purposes. Existing Talos CA material is not automatically converted.
...
`kubeconfig.yaml`. Existing reinstall enrollment mismatches must be resolved
explicitly with `katlctl context save --replace-node NODE`, after verifying the
replacement machine; the task does not silently overwrite those bindings.
...
References: [Katl configuration](https://github.com/katl-dev/katl/blob/v2026.9.0-beta.2/docs/installing.md),
